...
class DemoExceptionHandler(BaseExceptionHandler):
    """一个默认的异常处理器示例，可以根据状态码进行不同处理"""

    # 指定要处理的状态码
    handled_status_codes = [3006, 3007]
    ...
    def handle(
        ...
        """
        url = (
            f"{client_instance.base_url}/{request_config.get('endpoint', '').lstrip('/')}"
            if request_config.get("endpoint")
            else client_instance.base_url
        )
...
class BaseClient:
    ...
    def __init__(
        ...
        exception_handlers: BaseExceptionHandler
        | type[BaseExceptionHandler]
        | None = None,  # --- 新增：exception_handler 参数 ---
        **kwargs,
    ):
        """
        初始化客户端。
        ...
        self.exception_handler_instances = self._resolve_exception_handler(exception_handlers)
    ...
    def _resolve_exception_handler(
        self, exception_handler: BaseExceptionHandler | type[BaseExceptionHandler] | None
    ) -> BaseExceptionHandler | None:
        """解析异常处理器配置，返回处理器实例"""
        # 优先使用传入的处理器
        ...
        if isinstance(class_exception_handler, type) and issubclass(class_exception_handler, BaseExceptionHandler):
            ...
            raise APIClientValidationError(
                "exception_handler_class must be a BaseExceptionHandler subclass or instance"
            )
    ...
    def _make_request(self, request_id: str, request_config: dict[str, Any]) -> requests.Response:
        ...
        try:
            ...
            raise APIClientHTTPError(
                f"HTTP {status_code}: {e.response.reason if e.response else 'No response'}", response=e.response
            ) from e
...
if __name__ == "__main__":
    ...
    class HttpbinClientWithHandler(HttpbinClient):
        ...
        exception_handler_class = DemoExceptionHandler(handled_status_codes=[404], log_only_status_codes=[500])
    ...
    instance_handler = DemoExceptionHandler(handled_status_codes=[500])
