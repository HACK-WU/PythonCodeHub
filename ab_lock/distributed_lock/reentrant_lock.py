...
class ReentrantRedisLock(BaseLock):
    ...
    def acquire(self, wait: float = 0, retry_interval: float = 0.01) -> bool:
        ...
        while True:
            ...
            if self._is_acquire_success(result):
                self._local_count += 1
                return True
            # 非阻塞或已超时：直接返回失败
    ...
    @staticmethod
    def _is_acquire_success(result) -> bool:
        """容错解析 _ACQUIRE_LUA 返回值，兼容 None/bytes/str。"""
        if result is None:
            return False
        try:
            return int(result) == 1
    ...
    def release(self) -> int:
        ...
        ttl_ms = self.ttl * 1000
        raw = self.client.eval(_RELEASE_LUA, 1, self.name, self._token, ttl_ms)
        result = self._parse_release_result(raw)
        if result == -1:
    ...
    def _parse_release_result(self, raw) -> int:
        """容错解析 _RELEASE_LUA 返回值，兼容 None/bytes/str。

        异常返回（None 或非数值）一律按 -1 处理，触发本地计数清零，
        避免本地状态永久泄漏。
        """
        if raw is None:
            logger.warning(
                "unexpected _RELEASE_LUA result for %s: None; treating as -1",
                self.name,
            )
            return -1
        try:
            ...
            logger.warning(
                "unexpected _RELEASE_LUA result for %s: %r; treating as -1",
                self.name,
                raw,
            )
            return -1
